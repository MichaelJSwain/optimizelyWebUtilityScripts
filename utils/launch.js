...
if (args[2]) {
  const questions = [
    {
      ...
      message:
        "Are you sure you want to launch this experiment? It will be visible for real users (y|n)",
      validate: (val) => {
  ...
  prompt(questions).then(async (answers) => {
    ...
    if (launchConfirmed === "y") {
      // CHECK CUSTOM GOALS
      fs.readFile(
        `./experiments/${args[2]}/${args[3]}/sharedCode/shared.js`,
        { encoding: "utf8", flag: "r" },
        function (err, data) {
          if (err) console.log(err);
          else if (data.includes("optimizely.sendAnalyticsEvent")) {
            launch();
          } else {
            prompt(customGoalsQuestions).then(async (answers) => {
              if (answers.customGoalsConfirmation === "n") {
                console.log(
                  "Please check the custom goals in the Jira ticket, or reach out to an Analyst for help."
                );
              } else {
                launch();
              }
  ...
  console.log(
    "please specify the experiment ID for the experiment that you'd like launch"
  );
}
...
const launch = () => {
  ...
  console.log(
    "🚀 Experiment has been successfully launched! Please check that a PLC has been requested"
  );
};
